self.addEventListener("fetch",function(s){
    s.respondWith(caches.open("cache").then(
        function(e){return e.match(s.request).then(
            function(n){console.log("cache request: "+s.request.url);
            var t=fetch(s.request).then(
                function(n){return console.log("fetch completed: "+s.request.url,n),n&&(console.debug("updated cached page: "+s.request.url,n),
                "GET"===s.request.method&&"basic"===n.type&&e.put(s.request,n.clone())),n},function(s){console.log("Error in fetch()",s),
                s.waitUntil(caches.open("cache").then(function(s){return 
                    s.addAll(["/",
                    "/index.html",
                    ...
                    "/service-worker.js",
                    "/manifest.json"])}))});
                    return n||t})}))}),self.addEventListener("install",function(s){self.skipWaiting(),
